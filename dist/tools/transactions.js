...
const inputSchema = {
    address: z
        ...
        .max(200)
        .optional()
        .describe("Address to search around (recommended). Either 'address' or 'citycode' is required."),
    citycode: z
        .string()
...
export function registerTransactionsTool(server) {
    server.registerTool("fr_property_transactions", {
        ...
        try {
            ...
            if (args.response_format === ResponseFormat.JSON) {
                ...
                for (const s of sales) {
                    lines.push(`- **${s.date}** · ${s.propertyType} · ${eur(s.price)} · ${s.surfaceM2 ?? "?"} m² · ${s.rooms ?? "?"} p. · ${s.pricePerM2 ? eur(s.pricePerM2) + "/m²" : "n/a /m²"} · ${s.address}${s.commune ? ", " + s.commune : ""}`);
                }
                text = lines.join("\n");
            }
            return {
            ...
            return {
                content: [
                    { type: "text", text: formatError(error, "transactions") },
                ],
                isError: true,
            };
        }
