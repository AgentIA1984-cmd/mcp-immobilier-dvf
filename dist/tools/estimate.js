...
import { DEFAULT_RADIUS_M, MAX_RADIUS_M, COMP_SURFACE_TOLERANCE, MIN_COMPS_CONFIDENT, MIN_COMPS_LOW, } from "../constants.js";
const inputSchema = {
    address: z
        .string()
        .min(3)
...
export function registerEstimateTool(server) {
    server.registerTool("fr_property_price_estimate", {
...
Returns: estimate (EUR), low/high range (EUR), median/p25/p75 EUR/m², comps count, confidence ('high'|'medium'|'low'|'insufficient'). This is a statistical estimate, not a certified valuation.`,
        ...
        try {
            ...
            const comps = sales.filter((s) => s.pricePerM2 !== null &&
                s.surfaceM2 !== null &&
                s.surfaceM2 >= loB &&
                s.surfaceM2 <= hiB);
            const ppm2 = comps
            ...
            const n = ppm2.length;
            let confidence;
            if (n >= MIN_COMPS_CONFIDENT)
                confidence = "high";
            else if (n >= MIN_COMPS_LOW)
                confidence = "medium";
            else if (n >= 1)
                confidence = "low";
            else
            ...
            const output = {
                ...
                range_low_eur: low,
                range_high_eur: high,
                disclaimer: "Statistical estimate from DVF comparables — not a certified (expertise) valuation.",
            };
            let text;
            if (args.response_format === ResponseFormat.JSON) {
                ...
                text = [
                    ...
                    "",
                    `**Estimated value: ${eur(estimate)}** (range ${eur(low)} – ${eur(high)})`,
                    "",
            ...
            return {
                content: [
                    { type: "text", text: formatError(error, "price_estimate") },
                ],
                isError: true,
            };
        }
