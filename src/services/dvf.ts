...
export interface SaleQuery {
  citycode?: string; // INSEE commune code
  lat?: number;
  lon?: number;
  radiusM?: number; // used to filter by distance from (lat, lon)
  propertyType?: string; // "Maison" | "Appartement" | ... (substring match)
  sinceYear?: number; // keep sales on/after Jan 1 of this year
  minRooms?: number;
  maxRooms?: number;
  builtOnly?: boolean; // keep only records with a built surface (default true)
}
...
function normalizeRecord(rec: RawRecord): PropertySale {
  ...
  const addressParts = [numero, typeVoie, voie].filter(
    (p): p is string => !!p
  );
...
async function fetchRaw(query: SaleQuery): Promise<RawRecord[]> {
  ...
  if (query.citycode) {
    ...
    throw new Error(
      "A commune code (citycode) or a lat/lon pair is required to query DVF."
    );
  }
  const data = await httpGet<DvfApiResponse>(DVF_API_BASE, params);
 ...
 */
export async function getSales(query: SaleQuery): Promise<PropertySale[]> {
  ...
  if (
    ...
    sales = sales.filter((s) => {
      if (s.lat === null || s.lon === null) return true; // keep if uncoded
      return (
        haversineMeters(query.lat!, query.lon!, s.lat, s.lon) <= query.radiusM!
      );
    });
  }
  // Most recent first.
