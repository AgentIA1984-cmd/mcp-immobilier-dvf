...
export function registerMarketStatsTool(server: McpServer): void {
  server.registerTool(
...
Returns: salesCount, medianPricePerM2, p25/p75 EUR/m², medianPrice, and yoyPricePerM2Pct (last full year vs prior year, null if insufficient data).`,
    ...
    async (args) => {
      try {
        ...
        const ppm2ByYear = (y: number): number[] =>
          sales
        ...
        const yoy =
          medLast !== null && medPrev !== null && medPrev > 0
        ...
        if (args.response_format === ResponseFormat.JSON) {
          ...
          const trend =
            yoy === null ? "n/a" : `${yoy > 0 ? "+" : ""}${yoy}% YoY`;
          text = [
            `# Market stats — ${scopeLabel} · ${args.property_type}`,
            "",
            `- Sales analysed (since ${sinceYear}): ${stats.salesCount}`,
            `- Median: ${eur(stats.medianPricePerM2)}/m² (p25 ${eur(
              stats.p25PricePerM2
            )} – p75 ${eur(stats.p75PricePerM2)})`,
        ...
        return {
          content: [
            { type: "text" as const, text: formatError(error, "market_stats") },
          ],
          isError: true,
        };
      }
